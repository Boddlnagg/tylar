...
#[cfg(feature = "nightly")]
impl<T, N: PosType + NumType> TVec<T, N> {
    pub fn append<NR: PosType, NOut: PosType>(self, other: TVec<T, NR>) ->  TVec<T, NOut> where N: Add<NR, Out=NOut>  {
        let mut vec = self.vec;
        let mut other = other;
